...
function zen_draw_products_pull_down($name, $parameters = '', $exclude = '', $show_id = false, $set_selected = false, $show_model = false, $show_current_category = false, $enable = false) {
  ...
  if ($show_current_category) {
    if ($enable) {
      ...
      $products = $db->Execute("select p.products_id, pd.products_name, p.products_price, p.products_model, ptc.categories_id
              from " . TABLE_PRODUCTS . " p left join " . TABLE_PRODUCTS_TO_CATEGORIES . " ptc on ptc.products_id = p.products_id, " .
      TABLE_PRODUCTS_DESCRIPTION . " pd
              where p.products_id = pd.products_id
              and pd.language_id = '" . (int)$_SESSION['languages_id'] . "'
              and p.products_status = true and ptc.categories_id = '" . $current_category_id . "'
    ...
    if ($enable) {
      $products = $db->Execute("select p.products_id, pd.products_name, p.products_price, p.products_model
                    ...
                    where p.products_id = pd.products_id
                    and p.products_status = true and pd.language_id = '" . (int)$_SESSION['languages_id'] . "'
                    order by products_name");
    }
    else {
      $products = $db->Execute("select p.products_id, pd.products_name, p.products_price, p.products_model
