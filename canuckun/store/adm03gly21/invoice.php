  ...
  <tr>
    <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td class="pageHeading" align="right"><font style="font-size:20px"><?php echo ENTRY_ORDER_ID . $oID; ?></font><br><?php echo zen_image(DIR_WS_IMAGES . HEADER_LOGO_IMAGE_INVOICE, HEADER_ALT_TEXT); ?></td>
      </tr>
    </table></td>
  </tr>
  <tr>
...
<?php
  ...
  for ($i = 0, $n = sizeof($order->totals); $i < $n; $i++) {
    ...
    //  $format_shipping = explode(" (", $order->totals[$i]['title'], 2);
      $shipping_method = rtrim($order->totals[$i]['title'], ":"); //rtrim($format_shipping[0], ":");
      break;
    }
  }
?>
  ...
  <tr>
    <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr class="dataTableHeadingRow">
        ...
        <td class="dataTableHeadingContent"><?php echo TABLE_HEADING_ATTRIBUTES; ?></td>
...
<?php
    for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
      ...
      if (isset($order->products[$i]['attributes']) && (($k = sizeof($order->products[$i]['attributes'])) > 0)) {
        for ($j = 0; $j < $k; $j++) {
          echo '<nobr>' . $order->products[$i]['attributes'][$j]['option'] . ': ' . $order->products[$i]['attributes'][$j]['value'];
...
/*      echo '        <td class="dataTableContent" align="right" valign="top">';
      ...
      echo '</td>' . "\n" ;
*/    echo '        <td class="dataTableContent" valign="top" align="center">' . $order->products[$i]['qty'] . '</td>' . "\n" ;
    if ($display_tax) {
...
<?php if ($display_notes == 1) { ?>
  ...
  <tr>
    <td class="main" colspan="2"><table class="inv_comments" cellpadding="0" cellspacing="0"><tr><td><?php echo $customer_notes; ?></td></tr></table></td>
  </tr>
<?php } ?>  
...
<?php
  for ($i = 0, $n = sizeof($order->totals); $i < $n; $i++) {
	...
	if ($order->totals[$i]['class'] == 'ot_tax') {
		 $splitTax = splitTax($order->totals[$i]['title'], $order->totals[$i]['value']); 
		 while (list($desc,$tax) = each($splitTax)) {
      		echo '          <tr>' . "\n" .
           		 '            <td align="right" class="'. str_replace('_', '-', $order->totals[$i]['class']) . '-Text">' . trim(trim($desc), ':') . ':' . '</td>' . "\n" .
           		 '            <td align="right" class="'. str_replace('_', '-', $order->totals[$i]['class']) . '-Amount">' . $currencies->format($tax) . '</td>' . "\n" .
           		 '          </tr>' . "\n";
		 }
	}
	else {	  
...
<?php if (ORDER_COMMENTS_INVOICE > 0) { ?>
      <tr>
        <td class="main"><table border="1" cellspacing="0" cellpadding="5" class="inv_comments">
          <tr>
