...
*/
  ...
  if ($_GET['process'] == '1') {
    switch ($action) {
      ...
      case 'add':
        $update_status = false; //Do not update status //(isset($_GET['update_status']) ? $_GET['update_status'] : false);
        $notify_customer = false; //Do not notify customer //(isset($_GET['notify_customer']) ? $_GET['notify_customer'] : false);
        ...
        switch ($payment_mode) {
          case 'payment':
            // input new data
            $new_index = $so->add_payment($_GET['date_posted'], $_GET['payment_number'], $_GET['card_expiry'], $_GET['payment_name'], $_GET['payment_amount'], $_GET['payment_type'], $_GET['comments'], $_GET['purchase_order_id']);
            ...
            zen_redirect(zen_href_link(FILENAME_ORDER_PAYMENTS, 'oID=' . $so->oID . '&payment_mode=' . $payment_mode . '&index=' . $new_index . '&action=confirm', 'NONSSL'));
          break;
      ...
      case 'update';
        switch ($payment_mode) {
          case 'payment':
            $so->update_payment($_GET['payment_id'], $_GET['date_posted'], $_GET['purchase_order_id'], $_GET['payment_number'], $_GET['payment_name'], $_GET['payment_amount'], $_GET['payment_type'], $_GET['comments']);

            zen_redirect(zen_href_link(FILENAME_ORDER_PAYMENTS, 'oID=' . $so->oID . '&payment_mode=' . $payment_mode . '&index=' . $_GET['payment_id'] . '&action=confirm', 'NONSSL'));
          break;

          case 'purchase_order':
            $so->update_purchase_order($_GET['purchase_order_id'], $_GET['po_number']);

            zen_redirect(zen_href_link(FILENAME_ORDER_PAYMENTS, 'oID=' . $so->oID . '&payment_mode=' . $payment_mode . '&index=' . $_GET['purchase_order_id'] . '&action=confirm', 'NONSSL'));
          break;
          ...
          case 'refund':
            ...
            zen_redirect(zen_href_link(FILENAME_ORDER_PAYMENTS, 'oID=' . $so->oID . '&payment_mode=' . $payment_mode . '&index=' . $_GET['refund_id'] . '&action=confirm', 'NONSSL'));
          break;
        }  // END switch ($payment_mode)
...
/*      case 'delete':
        ...
        zen_redirect(zen_href_link(FILENAME_ORDER_PAYMENTS, 'oID=' . $so->oID . '&affected_rows=' . $affected_rows . '&action=delete_confirm', 'NONSSL'));
...
<?php
    switch ($action) {
      case 'add':
        echo '  ' . zen_draw_form('add', FILENAME_ORDER_PAYMENTS, '', 'get', '', true) . NL;
...
?>
    ...
    <tr>
      <td class="main" align="right"><?php echo TEXT_DATE_POSTED; ?></td>
      <td class="main"><?php echo zen_draw_input_field('date_posted', date('Y-m-d', time()), 'size="10"'); ?></td>
    </tr>
    <tr>
...
<?php
  /*
   <tr>
    ...
    <tr>
      <td class="main" align="right"><?php echo TEXT_CARD_EXPIRY; ?></td>
      <td class="main"><?php echo zen_draw_input_field('card_expiry', '', 'size="8" maxlength="4"'); ?></td>
    </tr>
   */
?>
    <tr>
...
-->    <tr>
      <td class="main" align="right"><?php echo TEXT_COMMENTS; ?></td>
      <td class="main"><?php echo zen_draw_input_field('comments', '', 'maxlength=50 size="50"'); ?></td>
    </tr>
 <?php
          break;
/*
...
<?php
      ...
      case 'update':
        ...
        echo '  ' . zen_draw_form('update', FILENAME_ORDER_PAYMENTS, '', 'get', '', true) . NL;
        echo '    ' . zen_draw_hidden_field('action', $action) . NL;
        echo '    ' . zen_draw_hidden_field('process', 1) . NL;
        echo '    ' . zen_draw_hidden_field('payment_mode', $payment_mode) . NL;
        echo '    ' . zen_draw_hidden_field('oID', $so->oID) . NL;
        ...
        switch ($payment_mode) {
          case 'payment':
            ...
            $payment = $db->Execute("select * from " . TABLE_YO_PAYMENTS . " where payment_id = '" . $index . "'");
            $po_array = $so->build_po_array(TEXT_NONE);
?>
    <tr>
    ...
    <tr>
      <td class="main" align="right"><?php echo TEXT_DATE_POSTED; ?></td>
      <td class="main"><?php echo zen_draw_input_field('date_posted', $payment->fields['date_posted'], 'size="10"'); ?></td>
    </tr>
    <tr>
      <td class="main" align="right"><?php echo TEXT_PAYMENT_TYPE; ?></td>
      <td class="main"><?php echo zen_draw_pull_down_menu('payment_type', $so->payment_key, $payment->fields['payment_type'], ''); ?></td>
      <input type="hidden" name="payment_name" value="<?php echo $payment->fields['payment_name']; ?>">
      <input type="hidden" name="payment_number" value="<?php echo $payment->fields['payment_number']; ?>">
      <input type="hidden" name="card_expiry" value="<?php echo $payment->fields['card_expiry'];?>">
    </tr>
<?php 
/*
...
-->
    <tr>
      <td class="main" align="right"><?php echo TEXT_COMMENTS; ?></td>
      <td class="main"><?php echo zen_draw_input_field('comments', $payment->fields['comments'], 'maxlength=50 size="50"'); ?></td>
    </tr>
 <?php
          break;
/*
...
/*
      case 'delete':
        ...
        $index = $_GET['index'];
        echo '  ' . zen_draw_form('delete', FILENAME_ORDER_PAYMENTS, '', 'get', '', true) . NL;
        echo '    ' . zen_draw_hidden_field('action', $action) . NL;
        echo '    ' . zen_draw_hidden_field('process', 1) . NL;
        echo '    ' . zen_draw_hidden_field('payment_mode', $payment_mode) . NL;
        echo '    ' . zen_draw_hidden_field('oID', $so->oID) . NL;
...
?>
    ...
    <tr>
      <td class="main" width="50%" align="right"><?php echo TEXT_DATE_POSTED; ?></td>
      <td class="main" width="50%" align="left"><strong><?php echo $payment_info->fields['date_posted']; ?></strong></td>
    </tr>
 <!--   <tr>
...
-->    <tr>
    ...
    <tr>
      <td class="main" width="50%" align="right"><?php echo TEXT_COMMENTS; ?></td>
      <td class="main" width="50%" align="left"><strong><?php echo $payment_info->fields['comments']; ?></strong></td>
    </tr>
<!--    <tr>
