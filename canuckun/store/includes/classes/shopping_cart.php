...
class shoppingCart extends base {
  ...
  function restore_contents() {
        ...
        if ($product->RecordCount()<=0) {
          $sql = "insert into " . TABLE_CUSTOMERS_BASKET . "
                                (customers_id, products_id, customers_basket_quantity,
                                 customers_basket_date_added,customers_basket_design)
                                 values ('" . (int)$_SESSION['customer_id'] . "', '" . zen_db_input($products_id) . "', '" .
          $qty . "', '" . date('Ymd') . "','".zen_db_input(base64_encode(serialize($this->contents[$products_id]['design_array'])))."')";
    ...
    $products_query = "select * from " . TABLE_CUSTOMERS_BASKET . " where customers_id = '" . (int)$_SESSION['customer_id'] . "'";

    $products = $db->Execute($products_query);

    while (!$products->EOF) {
      $this->contents[$products->fields['products_id']] = array('qty' => $products->fields['customers_basket_quantity'],'design_array'=>base64_decode(unserialize($products->fields['customers_basket_design'])));//Ìí¼ÓdesignÖÁsession
   ...
   */
  function add_cart($products_id, $qty = '1', $attributes = '', $notify = true, $design_array='') {
    global $db;
    $this->notify('NOTIFIER_CART_ADD_CART_START');
    $products_id = zen_get_uprid($products_id, $attributes,$design_array);
    if ($notify == true) {
    ...
    if ($this->in_cart($products_id)) {
      $this->update_quantity($products_id, $qty, $attributes,$design_array);
    } else {
      $this->contents[] = array($products_id);
      $this->contents[$products_id] = array('qty' => (float)$qty,'design_array'=>$design_array);
      ...
      if (isset($_SESSION['customer_id'])) {
        $sql = "insert into " . TABLE_CUSTOMERS_BASKET . "
                              ...
                              customers_basket_date_added,customers_basket_design)
                              values ('" . (int)$_SESSION['customer_id'] . "', '" . zen_db_input($products_id) . "', '" .
        $qty . "', '" . date('Ymd') . "','".zen_db_input(base64_encode(serialize($design_array)))."')";
   ...
   * @global object access to the db object
   */
  function update_quantity($products_id, $quantity = '', $attributes = '',$design_array='') {
    global $db;
    $this->notify('NOTIFIER_CART_UPDATE_QUANTITY_START');
    if (empty($quantity)) return true; // nothing needs to be updated if theres no quantity, so we return true..

    $this->contents[$products_id] = array('qty' => (float)$quantity,'design_array'=>$design_array);
    // update database
    if (isset($_SESSION['customer_id'])) {
      $sql = "update " . TABLE_CUSTOMERS_BASKET . "
  ...
  function calculate() {
    ...
    while (list($products_id, ) = each($this->contents)) {
      ...
      if ($product = $db->Execute($product_query)) {
        ...
        //  $products_price = zen_get_product_promotion_price($products_price);
        //}
        ...
        //assign final price by the price which is the least
        if ($group_price > 0 and $group_price < $products_price) {
          $products_price = $group_price;
        }

        if(isset($this->contents[$products_id]['design_array']['price'])) {
          $products_price += $this->contents[$products_id]['design_array']['price'];
        }
      ...
      if (isset($this->contents[$products_id]['attributes'])) {
        ...
        while (list($option, $value) = each($this->contents[$products_id]['attributes'])) {
          ...
          if ($attribute_price->fields['product_attribute_is_free'] == '1' and zen_get_products_price_is_free((int)$prid)) {
            ...
            $sql = "select * from " . TABLE_PRODUCTS_ATTRIBUTES_DOWNLOAD . " where products_attributes_id = '" . $check_attribute . "'";
  ...
  function get_products($check_for_valid_cart = false) {
    ...
    while (list($products_id, ) = each($this->contents)) {
      $products_query = "select p.products_id,  p.master_categories_id, p.products_status, pd.products_name, p.products_model, p.products_image,
                                  p.products_price, p.products_weight,  p.products_dim_type, p.products_weight_type,
      ...
      if ($products = $db->Execute($products_query)) {
        ...
        //get lowest price from the price list
        if ( $group_price >0 and $group_price < $products_price) {
          $products_price = $group_price;
        }
        ...
        //'quantity' => $this->contents[$products_id]['qty'],
                                  ...
                                  'weight' => $products->fields['products_weight'] + $this->attributes_weight($products_id),
                                    'weight_type' => $products->fields['products_weight_type'],
                                    'dim_type' => $products->fields['products_dim_type'],
                                    'length' => $products->fields['products_length'],
                                    'width' => $products->fields['products_width'],
                                    'height' => $products->fields['products_height'],
                                    'ready_to_ship' => $products->fields['products_ready_to_ship'],
                                  'final_price' => ($products_price + $this->attributes_price($products_id)+$this->contents[$products_id]['design_array']['price']),
								  ...
								  'design_array'=>(isset($this->contents[$products_id]['design_array']) ? $this->contents[$products_id]['design_array'] : ''));
      }
    }
  ...
  function actionUpdateProduct($goto, $parameters) {
    ...
    $_SESSION['promotion_products'] = $_POST['promotion_code'];
    //}
