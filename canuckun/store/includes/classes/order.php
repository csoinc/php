...
class order extends base {
  ...
  function query($order_id) {
    ...
    $totals_query = "select title, text, value, class
  ...
  function cart() {
    ...
    for ($i=0, $n=sizeof($products); $i<$n; $i++) {
      ...
      //--------------------------------
      if (defined('DISPLAY_SPLIT_TAX_LINES') && (DISPLAY_SPLIT_TAX_LINES == 'true'))
      {
        $this->products[$index] = array('qty' => $products[$i]['quantity'],
                                            'name' => $products[$i]['name'],
                                            'model' => $products[$i]['model'],
                                            'tax' => zen_get_multiple_tax_rates($products[$i]['tax_class_id'], $tax_address->fields['entry_country_id'], $tax_address->fields['entry_zone_id']),
                                            'tax_description' => zen_get_multiple_tax_rates($products[$i]['tax_class_id'], $tax_address->fields['entry_country_id'], $tax_address->fields['entry_zone_id']),
                                            'price' => $products[$i]['price'],
                                            'final_price' => $products[$i]['price'] + $_SESSION['cart']->attributes_price($products[$i]['id'])+$products[$i]['design_array']['price'],
                                            ...
                                            'id' => $products[$i]['id'],
                                            'rowClass' => $rowClass,
      				            'design_array' => $products[$i]['design_array']);
      }
      else
      {
        $this->products[$index] = array('qty' => $products[$i]['quantity'],
      ...
      //end SplitTaxLines modification
      //------------------------------
      ...
      *********************************************/
      ...
      if (defined('DISPLAY_SPLIT_TAX_LINES') && (DISPLAY_SPLIT_TAX_LINES == 'true'))
      {
        ...
        foreach ($products_tax as $tax_entry_description=>$tax_entry_rate)
        {
          $aggregate_tax_rate += $tax_entry_rate;
        }

        $shown_price = (zen_add_tax($this->products[$index]['final_price'], $aggregate_tax_rate) * $this->products[$index]['qty'])
        + zen_add_tax($this->products[$index]['onetime_charges'], $aggregate_tax_rate);
        ...
        foreach ($products_tax as $tax_entry_description=>$tax_entry_rate)
        {
          if (DISPLAY_PRICE_WITH_TAX == 'true') {
            ...
            $tax_add = $shown_price - ($shown_price / (($tax_entry_rate < 10) ? "1.0" . str_replace('.', '', $tax_entry_rate) : "1." . str_replace('.', '', $tax_entry_rate)));
          } else {
            // calculate the amount of tax for this product (assuming tax is NOT included in the price)
            $tax_add = zen_round(($tax_entry_rate / 100) * $shown_price, $currencies->currencies[$this->info['currency']]['decimal_places']);
          }
  ...
  function create($zf_ot_modules, $zf_mode = 2) {
    ...
    //Add vip code when empty
    if ($this->info['coupon_code'] == '') {
      $this->info['coupon_code'] = $_SESSION['promotion_products'];
    }    
  ...
  function  create_add_products($zf_insert_id, $zf_mode = false) {
    ...
    for ($i=0, $n=sizeof($this->products); $i<$n; $i++) {
      ...
      {
        $effective_tax = 0;
        foreach($this->products[$i]['tax'] as $key=>$value)
        {
          $effective_tax += $value;
        }
        $sql_data_array = array('orders_id' => $zf_insert_id,
      							  ...
      							  'design'=>zen_db_input(base64_encode(serialize($this->products[$i]['design_array']))));
      }
      else
      {
        $sql_data_array = array('orders_id' => $zf_insert_id,
