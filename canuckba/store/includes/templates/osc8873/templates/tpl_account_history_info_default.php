...
?>
		     <div class="amount larger forward"><?php echo $currencies->format($tax); ?></div>
     		 <div class="lineTitle larger forward"><?php echo trim(trim($desc), ':') . ':'; ?></div>
			 <br class="clearBoth" />
<?php 
		 }
	}
...
?>
    <tr>
        <td width=90><?php echo zen_date_short($statuses['date_added']); ?></td>
        <td width=90><?php echo $statuses['orders_status_name']; ?></td>
        <td><?php echo (empty($statuses['comments']) ? '&nbsp;' : '<table class="inv_comments" cellpadding="0" cellspacing="0"><tr><td>' . nl2br(zen_output_string_protected($statuses['comments'])) . '</td></tr></table> ' ); ?></td> 
     </tr>
<?php
  }
?>
...
<div style="font-size: em0.9; color:#666">*If you have any questions on your order, <a href="<?php echo zen_href_link(FILENAME_CONTACT_US, '', 'NONSSL') ?>">please contact us.</a></div>
<div class="buttonRow forward"><?php echo '<a href="' . zen_href_link(FILENAME_ACCOUNT_HISTORY_INFO, '', 'SSL') . '">' . zen_image_button(BUTTON_IMAGE_BACK, BUTTON_BACK_ALT) . '</a>'; ?></div>