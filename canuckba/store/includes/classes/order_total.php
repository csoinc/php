...
class order_total extends base {
  ...
  function output($return_html=false) {
    global $template, $current_page_base, $currencies;
    $output_string = '';
    if (is_array($this->modules)) {
      reset($this->modules);
      while (list(, $value) = each($this->modules)) {
        ...
        if ($return_html == true) {
          for ($i=0; $i<$size; $i++) {
            print $GLOBALS[$class]->code . '<br>';
            if ($GLOBALS[$class]->code == 'ot_tax') {
              $splitTax = splitTax($GLOBALS[$class]->output[$i]['title'], $GLOBALS[$class]->output[$i]['value']);
              while (list($desc,$tax) = each($splitTax)) {
                $output_string .= '              <tr>' . "\n" .
	           		 '                <td align="right" class="' . str_replace('_', '-', $GLOBALS[$class]->code) . '-Text">' . trim(trim($desc), ':') . ':' . '</td>' . "\n" .
            			'                <td align="right" class="' . str_replace('_', '-', $GLOBALS[$class]->code) . '-Amount">' . $currencies->format($tax) . '</td>' . "\n" .
            '              </tr>';
              }
