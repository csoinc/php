...
class Stocks extends CI_Model {
  ...
  function insert_stock($itemcode, $styleid, $code, $xsmall, $small, $medium, $large, $xlarge, $xxlarge, $comments)
  {
    ...
    if ($stockid) {
      ...
      .$stockid.",'".$itemcode."',".$styleid.",'".$code."',".$xsmall.",".$small.",".$medium.",".$large.",".$xlarge.",".$xxlarge.",'".mysql_real_escape_string($comments).
      "',0,now())";
  ...
  function insert_order_item($itemcode, $styleid, $code, $orderid, $description)
  {
    ...
    if ($stockid) {
      ...
      .$stockid.",'".$itemcode."',".$styleid.",'".$code."',".$orderid.",'".mysql_real_escape_string($description)
      ."',now())";
      //log_message('debug', $insert_sql);
      $query = $this->db->query($insert_sql);
      if ($this->db->affected_rows() > 0) {
  ...
  {
    $update_sql="UPDATE stocks SET xsmallnumbers='".mysql_real_escape_string($xsmallnumbers)
    ."',smallnumbers='".mysql_real_escape_string($smallnumbers)
    ."',mediumnumbers='".mysql_real_escape_string($mediumnumbers)
    ."',largenumbers='".mysql_real_escape_string($largenumbers)
    ."',xlargenumbers='".mysql_real_escape_string($xlargenumbers)
    ."',xxlargenumbers='".mysql_real_escape_string($xxlargenumbers)
    ."' WHERE stockid = ".$stockid;
  ...
  {
    $update_sql="UPDATE stocks SET description='".mysql_real_escape_string($description)
    ."',styleid=".$styleid
    ." WHERE stockid = ".$stockid;
