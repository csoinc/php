...
function zen_get_products_special_price($product_id, $specials_price_only=false) {
  ...
  if ($specials->RecordCount() > 0) {
    ...
    if (zen_check_promotion_code()) {
      $special_price = zen_get_product_promotion_price($special_price);
    }
  } else {
    if (zen_check_promotion_code()) {
      $special_price = zen_get_product_promotion_price($product_price);
    } else {
      $special_price = false;
    }
  }
  ...
  if(substr($product->fields['products_model'], 0, 4) == 'GIFT') {
    //Never apply a salededuction to Giftvouchers
...
// Specials and Tax Included
// in a fancy style
function zen_get_products_display_price_complex($products_id) {
  global $db, $currencies;
  ...
  if (SHOW_SALE_DISCOUNT_STATUS == '1' and ($display_special_price != 0 or $display_sale_price != 0)) {
    if ($display_sale_price) {
      if (SHOW_SALE_DISCOUNT == 1) {
        ...
        $show_sale_discount = '<td class="productPriceDiscountEx">' . PRODUCT_PRICE_DISCOUNT_PREFIX_EX . $show_discount_amount . PRODUCT_PRICE_DISCOUNT_PERCENTAGE . '</td>';

      } else {
        $show_sale_discount = '<td class="productPriceDiscountEx">' . PRODUCT_PRICE_DISCOUNT_PREFIX_EX . $currencies->display_price(($display_normal_price - $display_sale_price), zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . PRODUCT_PRICE_DISCOUNT_AMOUNT . '</td>';
      }
    } else {
      if (SHOW_SALE_DISCOUNT == 1) {
        $show_sale_discount = '<td class="productPriceDiscountEx">' . PRODUCT_PRICE_DISCOUNT_PREFIX_EX . number_format(100 - (($display_special_price / $display_normal_price) * 100),SHOW_SALE_DISCOUNT_DECIMALS) . PRODUCT_PRICE_DISCOUNT_PERCENTAGE . '</td>';
      } else {
        $show_sale_discount = '<td class="productPriceDiscountEx">' . PRODUCT_PRICE_DISCOUNT_PREFIX_EX . $currencies->display_price(($display_normal_price - $display_special_price), zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . PRODUCT_PRICE_DISCOUNT_AMOUNT . '</td>';
      }
    }
  }

  if ($display_special_price) {
    $show_normal_price = '<td class="normalpriceEx">' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . ' </td>';
    if ($display_sale_price && $display_sale_price != $display_special_price) {
      $show_special_price = '<td class="productSpecialPriceSaleEx">' . $currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</td>';
      if ($product_check->fields['product_is_free'] == '1') {
        $show_sale_price = '<td class="productSalePriceEx">' . PRODUCT_PRICE_SALE_EX . '<s>' . $currencies->display_price($display_sale_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</s>' . '</td>';
      } else {
        $show_sale_price = '<td class="productSalePriceEx">' . PRODUCT_PRICE_SALE_EX . $currencies->display_price($display_sale_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</td>';
      }
    } else {
      if ($product_check->fields['product_is_free'] == '1') {
        $show_special_price = '<td class="productSpecialPriceEx">' . '<s>' . $currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</s>' . '</td>';
      } else {
        $show_special_price = '<td class="productSpecialPriceEx">' . $currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</td>';
      }
      $show_sale_price = '';
    }
  } else {
    if ($display_sale_price) {
      ...
      if ($product_check->fields['product_is_free'] == '1') {
        $show_normal_price = '<td class="productFreeEx">' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</td>';
      } else {
        $show_normal_price = '<td class="normalpriceEx">' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</td>';
      }
      $show_special_price = '';
  ...
  if ($display_normal_price == 0) {
    // don't show the $0.00
    $final_display_price = '<table class="priceList"><tr>' . $show_special_price . $show_sale_price . $show_sale_discount . '</tr></table>';
  } else {
    $final_display_price = '<table class="priceList"><tr>' . $show_normal_price . $show_special_price . $show_sale_price . $show_sale_discount . '</tr></table>';
  }
...
function zen_get_product_promotion_price($price)
{
  ...
  if (preg_match("/\.99/", $price)) {
    ...
    $promotion_price = $price * 1.00;
  }
  return number_format($promotion_price, 2, '.', '');
}

function zen_check_promotion_code() {
  $vip_code = $_SESSION['promotion_products'];
  if (strlen($vip_code) < 4) return false;
  ...
  "VBC",
  "VBCCoachCertificate",
  "VBCRefereeCertificate",
  "OURVIPHSPlayer",
  "CrazyAboutVolleyball",
  "CCVA",
  "GuelphGrizzlies",
  "NVVIP",
  "velocity",
  ...
  foreach ($codes as $code) {
    if ($code == $vip_code) return TRUE;
  }
  //return preg_match("/" . $vip_code . "/", implode('.', $codes));
  return FALSE;
}
