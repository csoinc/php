...
<div class="centerColumn" id="checkoutConfirmDefault">
	<h1 id="checkoutConfirmDefaultHeading">
...
<div class="buttonRow forward"><?php echo  '<a href="' . zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL') . '">' . zen_image_button(BUTTON_IMAGE_EDIT_SMALL, BUTTON_EDIT_SMALL_ALT) . '</a>'; ?></div>
<div><?php echo (empty($order->info['comments']) ? NO_COMMENTS_TEXT : '<table class="inv_comments"><tr><td>' . nl2br(zen_output_string_protected($order->info['comments'])) . zen_draw_hidden_field('comments', $order->info['comments'])) . '</td></tr></table>'; ?></div>
<br class="clearBoth" />
<?php
//  }
?>
...
{
	$effective_tax = 0;
	foreach($order->products[$i]['tax'] as $description=>$rate)
	{
		$effective_tax+= $rate;
	}
?>
<?php echo zen_display_tax_value($effective_tax); ?>%</td>
<?php
}
else 
{
...
?>
<p><?php echo TEXT_SHIPPING_FEE_NOTICE; ?></p>
