...
if ($_GET['action']=='save'){
  // if changes to adminID's profile are being saved
  ...
  while (!$headers->EOF){
    ...
    if ($field == 'on' || $field == 'off'){
      if ($althere->fields['header_id'] == ''){
        $query = $db->Execute("insert into ".TABLE_ADMIN_VISIBLE_HEADERS." set header_id = '".$headers->fields['id']."', admin_id = '".$_GET['adminID']."'");
      }
    }else{
      if ($althere->fields['header_id'] != ''){
        $query = $db->Execute("delete from ".TABLE_ADMIN_VISIBLE_HEADERS." where header_id = '".$headers->fields['id']."' and admin_id = '".$_GET['adminID']."'");
      }
    }
    $headers->MoveNext();
  }
  ...
  while (!$pages->EOF){
    ...
    if ($field == 'on' || $field == 'off'){
      if ($althere->fields['page_id'] == ''){
        $query = $db->Execute("insert into ".TABLE_ADMIN_ALLOWED." set page_id = '".$pages->fields['id']."', admin_id = '".$_GET['adminID']."'");
      }
    }else{
      if ($althere->fields['page_id'] != ''){
        $query = $db->Execute("delete from ".TABLE_ADMIN_ALLOWED." where page_id = '".$pages->fields['id']."' and admin_id = '".$_GET['adminID']."'");
      }
    }
    $pages->MoveNext();
  }
...
<link rel="stylesheet" type="text/css"
	href="includes/admin_profiles.css">
...
?>
	...
	<div id="profileHeader">
		...
		<?php echo 'Admin Profile for '.$admin->fields['admin_name'] ?></div>
		<div id="profileFunctions">
			<a href="<?php echo zen_href_link(FILENAME_ADMIN)?>">Update another
				user</a>
		</div>
	</div>
	<form id="profileBoxes" name="profileBoxes"
		action="admin_control.php?adminID=<?php echo $adminID; ?>&amp;action=save"
		method="post">
		<div class="formButtons">
			<input type="submit" value="Save Changes"><input type="reset"
