...
// Retreive shipping Tax Rate/Percent
// modified by huangyan
    $shipping_module_query = $db -> Execute("select shipping_module_code from " . TABLE_ORDERS . " where orders_id=" . $_GET['oID'] );
    $shipping_tax_class_id_query = $db -> Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key= 'MODULE_SHIPPING_" . $shipping_module_query->fields['shipping_module_code'] . "_TAX_CLASS'" );
    $shipping_tax_class_basis_query = $db -> Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key= 'MODULE_SHIPPING_" . $shipping_module_query->fields['shipping_module_code'] . "_TAX_BASIS'" );

	$shipping_tax_class_id = -1;
	if (count($shipping_tax_class_id_query)==1) {
		$shipping_tax_class_id = $shipping_tax_class_id_query->fields['configuration_value'];
	}

	if ( $shipping_tax_class_basis_query->fields['configuration_value'] == 'shipping' ) {
		$CountryID2 = zen_get_country_id($_POST['update_delivery_country']);
		$ZoneID2 = zen_get_zone_id($CountryID2,  $_POST['update_delivery_state']);
	}
	else {
		$CountryID2 = zen_get_country_id($_POST['update_billing_country']);
		$ZoneID2 = zen_get_zone_id($CountryID2,  $_POST['update_billing_state']);
	}
	$AddShippingTax = zen_get_tax_rate($shipping_tax_class_id, $CountryID2, $ZoneID2);
//print "ctr:". $_POST['update_delivery_country'] ."  state:" . $_POST['update_delivery_state'] . " tax:" . $shipping_tax_class_id ."!";

//// end of shipping tax rate block 
				...
				if( trim($ot_title) && trim($ot_value) )
				{
					...
					// Update ot_subtotal, ot_tax, and ot_total classes
						...
						// echo "ot_value = $ot_value<br>\n";
							...
							//update taxes in table 'orders'
							$Query = "update " . TABLE_ORDERS . " set
									order_tax = " . number_format($RunningTax, 2, '.', '') . " 
									where orders_id=$oID";
							$db -> Execute($Query);
						...
						if($ot_class == "ot_total")
						{
							...
							//update total in table 'orders'
							$Query = "update " . TABLE_ORDERS . " set
								order_total = " . number_format($RunningTotal, 2, '.', '') . "
								where orders_id=$oID";
							$db -> Execute($Query);
				
							//update balance_due in table 'orders'
							$Query = "update " . TABLE_ORDERS . " set
								balance_due = " . number_format($RunningTotal, 2, '.', '') . " - amount_paid
								where orders_id=$oID";
							$db -> Execute($Query);
...
$Query = "insert into " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . " set
						...
						products_options_values = '" . str_replace("'","''",$attributes[$i]) . "',
...
/* if(IsSet($add_product_options))
			...
			$db -> Execute($Query);
				...
				//update balance_due in table 'orders'
			$Query = "update " . TABLE_ORDERS . " set
				balance_due = " . number_format($Total, 2, '.', '') . " - amount_paid
				where orders_id=$oID";
			$db -> Execute($Query);
...
<!-- Begin Addresses Block -->
  ...
  <tr>
    ...
    <td valign="top"><span class="main">
      <input name="update_customer_postcode" size="25" value="<?php echo zen_html_quotes($order->customer['postcode']); ?>">
    </span></td>
    <td valign="top"><span class="main">
      <input name="update_billing_postcode" size="25" value="<?php echo zen_html_quotes($order->billing['postcode']); ?>">
    </span></td>
    <td valign="top"><span class="main">
      <input name="update_delivery_postcode" size="25" value="<?php echo zen_html_quotes($order->delivery['postcode']); ?>">
    </span></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
	...
	<!-- Begin Order Total Block -->
	  <tr>
	    <td align="right" colspan="6">
	    	...
	    	<tr>
	    	<td align='center' valign='top'><br><a href="<?php echo $PHP_SELF . "?oID=$oID&action=add_product&step=1"; ?>"><u><b><font size='3'><?php echo TEXT_DATE_ORDER_ADDNEW; ?> </font></b></u></a><br>Important: To calculate shipping tax correctly, please <font color="#FF0000">click UPDATE button again</font> after adding all products!!!</td>
...
?>
            ...
            <tr>
            	<td></td><td align="right"><B>Amount Paid:</B> <span style="background-color:<?php 
				if ($TotalDetails["Price"]==$order->info['amount_paid']) {
					echo "green";
				}
				else if ($TotalDetails["Price"] > $order->info['amount_paid']) {
				    echo "red";
				}
				else {
					echo "blue";
				}?>; color:#FFFFFF; font-size: 2em; width:150px; text-align:right;"><?php echo $order->info['amount_paid']; ?></span></td>
            </tr>
	    	</table>
	    </td>
	  </tr>
...
<?php
    ...
    if ($orders_history_query->RecordCount()) {
      ...
      while (!$orders_history_query -> EOF){
        ...
        /* if($CommentsWithStatus) { */
        echo '            <td class="smallText"><table cellpadding=0 cellspacing=0 class="inv_comments"><tr><td>' . nl2br(zen_db_output($orders_history_query->fields['comments'])) . '&nbsp;</td></tr></table></td>' . "\n";
       /* } */
...
?>
	...
	?>
      ...
      <tr>
	<td align='center' valign="top"><?php echo zen_image_submit('button_update.gif', IMAGE_UPDATE); 
	echo ' &nbsp; &nbsp; <a href="' . zen_href_link(FILENAME_ORDERS_INVOICE, 'oID=' . $_GET['oID']) . '" TARGET="_blank">' . zen_image_button('button_invoice.gif', IMAGE_ORDERS_INVOICE) . '</a> <a href="' . zen_href_link(FILENAME_ORDERS_PACKINGSLIP, 'oID=' . $_GET['oID']) . '" TARGET="_blank">' . zen_image_button('button_packingslip.gif', IMAGE_ORDERS_PACKINGSLIP) . '</a>'
	 ?>
     </td>
      </tr>
  </form>  
