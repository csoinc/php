...
class yan_order {
  ...
  function start() {
    ...
    $amount_paid = $order_query->fields['amount_paid'];
    ...
    $payment_key_query = $db->Execute("select * from " . TABLE_YO_PAYMENT_TYPES . "
    ...
    $payments_query = $db->Execute("select * from " . TABLE_YO_PAYMENTS . "
    ...
    if (zen_not_null($payments_query->fields['orders_id'])) {
      while (!$payments_query->EOF) {
        $this->payment[] = array('index' => $payments_query->fields['payment_id'],
                                 ...
                                 'comments' => $payments_query->fields['comments'],
    ...
    $purchase_order_query = $db->Execute("select * from " . TABLE_YO_PURCHASE_ORDERS . "
                                          where orders_id = '" . $this->oID . "'
                                          order by date_posted asc");
    ...
    if($this->purchase_order) {    // need a po before you can have po payments
      for($i = 0; $i < sizeof($this->purchase_order); $i++) {
        ...
        $po_payments_query = $db->Execute("select * from " . TABLE_YO_PAYMENTS . "
          ...
          while (!$po_payments_query->EOF) {
            $this->po_payment[] = array('index' => $po_payments_query->fields['payment_id'],
                                        ...
                                        'type' => $po_payments_query->fields['payment_type'],
                                 		'comments' => $po_payments_query->fields['comments'],
    ...
    if($this->payment || $this->po_payment) {   // gotta have payments in order to refund them
      $refunds_query = $db->Execute("select * from " . TABLE_YO_REFUNDS . "
                                     where orders_id = '" . $this->oID . "'
                                     order by date_posted asc");
    ...
    // compare this amount to the one stored in the orders table, update if necessary
    if ($this->amount_applied != $order_query->fields['amount_paid']) $this->new_amount_paid();
  ...
  // input the current value of $this->balance_due into balance_due
  // field in the orders table
  function new_amount_paid() {
    $b['amount_paid'] = $this->amount_applied;
    zen_db_perform(TABLE_ORDERS, $b, 'update', 'orders_id = ' . $this->oID);
  }
  ...
  function cc_line_item() {
    ...
    zen_db_perform(TABLE_YO_PAYMENTS, $new_cc_payment);
  }
  ...
  function build_po_array($include_blank = false) {
    ...
    $po_query = $db->Execute("select purchase_order_id, po_number from " . TABLE_YO_PURCHASE_ORDERS . " where orders_id = '" . $this->oID . "'");
  ...
  function build_payment_array($include_blank = false) {
    ...
    $payment_query = $db->Execute("select payment_id, payment_number from " . TABLE_YO_PAYMENTS . " where orders_id = '" . $this->oID . "'");
  ...
  function button_add($payment_mode) {
    ...
    zen_href_link(FILENAME_ORDER_PAYMENTS, 'oID=' . $this->oID . '&payment_mode=' . $payment_mode . '&action=add', 'NONSSL') . '\', \'scrollbars=yes,resizable=yes,width=400,height=300,screenX=150,screenY=100,top=100,left=150\')">' .
  ...
  function button_update($payment_mode, $index) {
    echo '&nbsp;<a href="javascript:popupWindow(\'' .
    zen_href_link(FILENAME_ORDER_PAYMENTS, 'oID=' . $this->oID . '&payment_mode=' . $payment_mode . '&index=' . $index . '&action=update', 'NONSSL') . '\', \'scrollbars=yes,resizable=yes,width=400,height=300,screenX=150,screenY=100,top=100,left=150\')">' .
  ...
  function button_delete($payment_mode, $index) {
    echo '&nbsp;<a href="javascript:popupWindow(\'' .
    zen_href_link(FILENAME_ORDER_PAYMENTS, 'oID=' . $this->oID . '&payment_mode=' . $payment_mode . '&index=' . $index . '&action=delete', 'NONSSL') . '\', \'scrollbars=yes,resizable=yes,width=400,height=300,screenX=150,screenY=100,top=100,left=150\')">' .
    zen_image_button('btn_remove.gif', sprintf(ALT_TEXT_DELETE, str_replace('_', ' ', $payment_mode))) . '</a>';
  }



  function add_payment($date_posted, $payment_number, $card_expiry, $payment_name, $payment_amount, $payment_type, $comments, $purchase_order_id = false) {

    $new_payment = array('orders_id' => $this->oID,
                         'payment_number' => zen_db_prepare_input($payment_number),
                         'card_expiry' => zen_db_prepare_input($card_expiry),
                         ...
                         'comments' => zen_db_prepare_input($comments),
						 'date_posted' => zen_db_prepare_input($date_posted),
						 'last_modified' => zen_db_prepare_input($date_posted));
    ...
    zen_db_perform(TABLE_YO_PAYMENTS, $new_payment);
  ...
  function update_payment($payment_id, $date_posted, $purchase_order_id = false, $payment_number = false, $payment_name = false, $payment_amount = false, $payment_type = false, $comments = '', $orders_id = false) {
    $update_payment = array();
    $update_payment['date_posted'] = $date_posted;
    ...
    if ($comments) {
      $update_payment['comments'] = zen_db_prepare_input($comments);
    }
    if (is_numeric($purchase_order_id)) {
      $update_payment['purchase_order_id'] = (int)$purchase_order_id;
    }

    zen_db_perform(TABLE_YO_PAYMENTS, $update_payment, 'update', "payment_id = '" . $payment_id . "'");
  }
  ...
  function add_purchase_order($po_number) {
    ...
    zen_db_perform(TABLE_YO_PURCHASE_ORDERS, $add_po);
  ...
  function update_purchase_order($purchase_order_id, $po_number = false, $orders_id = false) {
    ...
    if ($po_number && $po_number != '') {
     ...
     zen_db_perform(TABLE_YO_PURCHASE_ORDERS, $update_po, 'update', "purchase_order_id = '" . $purchase_order_id . "'");
  }
  ...
  function add_refund($payment_id, $refund_number, $refund_name, $refund_amount, $refund_type) {
    ...
    zen_db_perform(TABLE_YO_REFUNDS, $new_refund);
  ...
  function update_refund($refund_id, $payment_id = false, $refund_number = false, $refund_name = false, $refund_amount = false, $refund_type = false, $orders_id = false) {
    ...
    zen_db_perform(TABLE_YO_REFUNDS, $update_refund, 'update', "refund_id = '" . $refund_id . "'");
  }
  ...
  function delete_refund($refund_id, $payment_id = false, $all = false) {
    global $db;
    $db->Execute("delete from " . TABLE_YO_REFUNDS . " where refund_id = '" . $refund_id . "' limit 1");
  }
  ...
  function delete_payment($payment_id) {
    global $db;
    $db->Execute("delete from " . TABLE_YO_PAYMENTS . " where payment_id = '" . $payment_id . "' limit 1");
  }
  ...
  function delete_purchase_order($purchase_order_id) {
    global $db;
    $db->Execute("delete from " . TABLE_YO_PURCHASE_ORDERS . " where purchase_order_id = '" . $purchase_order_id . "' limit 1");
  }
  ...
  function delete_all_data() {
    global $db;
    // remove payment data
    $db->Execute("delete from " . TABLE_YO_PAYMENTS . " where orders_id = '" . $this->oID . "'");
    // remove purchase order data
    $db->Execute("delete from " . TABLE_YO_PURCHASE_ORDERS . " where orders_id = '" . $this->oID . "'");
    // remove refund data
    $db->Execute("delete from " . TABLE_YO_REFUNDS . " where orders_id = '" . $this->oID . "'");
  }
