      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
         <tr><?php echo zen_draw_form('search', FILENAME_ORDERS, '', 'get', '', true); ?>
            <td width="35%" class="pageHeading" align="right"><?php echo zen_draw_separator('pixel_trans.gif', 1, HEADING_IMAGE_HEIGHT); ?></td>
...
?>
         ...
         <?php echo zen_draw_form('search_orders_products', FILENAME_ORDERS, '', 'get', '', true); ?>
            ...
            <td colspan="2" class="smallText" align="right">
            From: <input type="text" name="date_from" id="date_from" value="2000-1-1"> <br>
            To: <input type="text" name="date_from" id="date_from" value=""> <br>
<?php
...
?>
      <tr>
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="pageHeading"><?php echo HEADING_TITLE . ' ( No.' . $oID . ' )'; ?></td>
            <td class="pageHeading" align="right"><?php echo zen_draw_separator('pixel_trans.gif', 1, HEADING_IMAGE_HEIGHT); ?></td>
      ...
      <tr>
        <td class="main"><strong><?php echo ENTRY_ORDER_ID . $oID; ?></strong> 
        	(<a href="<?php echo zen_href_link("orders_design","orders_id=".$oID);?>" target="_blank">See Custom Designed Uniforms) 
        </td>
      </tr>
      <tr>
...
// end: sbA
      ...
      echo '            </td>' . "\n" .
           ...
           '            <td class="dataTableContent" align="right" valign="top"><strong>' .
                          ...
                          ($order->products[$i]['onetime_charges'] != 0 ? '<br />' . $currencies->format($order->products[$i]['onetime_charges'], true, $order->info['currency'], $order->info['currency_value']) : '') .
                        '</strong></td>' . "\n";
      echo '          </tr>' . "\n"; //. zen_add_tax(1, $order->products[$i]['tax']) ;
    }
?>
          <tr>
...
<?php
    ...
    for ($i = 0, $n = sizeof($order->totals); $i < $n; $i++) {
      echo '              <tr>' . "\n" .
           ...
           '              </tr>' . "\n";
      $order_total_amount = ($order->totals[$i]['class'] == 'ot_total' ? $order->totals[$i]['value'] : 0);
    }
?>
            </table></td>
          </tr>
...
?>
      ...
      <tr>
        <td class="main"><table border="1" cellspacing="0" cellpadding="5" width="600">
		  <tr><td colspan="4"><b>Payments</b></td>
             <td colspan="1" align="right">
<?php echo '        <a href="javascript:popupWindow(\'' .
                   zen_href_link('order_payments', 'oID=' . $oID . '&action=add&payment_mode=payment', 'NONSSL') . '\', \'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=no,width=650,height=450,screenX=150,screenY=100,top=100,left=150\')">' . BUTTON_ADD_PAYMENT . '</a> ';
?>
          </td></tr>
          <tr>
            <td class="smallText" align="center" width="80"><strong><?php echo TABLE_HEADING_DATE_POSTED; ?></strong></td>
           <!--  <td class="smallText" align="center"><strong><?php echo TABLE_HEADING_DATE_MODIFIED; ?></strong></td> -->	
          <!--  <td class="smallText" align="center"><strong><?php echo TABLE_HEADING_PAYMENT_NAME; ?></strong></td>  -->
            <td class="smallText" align="center" width="120"><strong><?php echo TABLE_HEADING_PAYMENT_TYPE; ?></strong></td>
          <!--  <td class="smallText" align="center"><strong><?php echo TABLE_HEADING_PAYMENT_NUMBER; ?></strong></td>
            <td class="smallText" align="center"><strong><?php echo TABLE_HEADING_CARD_EXPIRY; ?></strong></td>  -->
            <td class="smallText" align="center" width="80"><strong><?php echo TABLE_HEADING_PAYMENT_AMOUNT; ?></strong></td>
            <td class="smallText" align="center"><strong><?php echo TABLE_HEADING_COMMENTS; ?></strong></td>
            <td class="smallText" align="center" width="100"><strong><?php echo TABLE_HEADING_PAYMENT_ACTION; ?></strong></td>
          </tr>
<?php
    $orders_payments_total = 0;
	//get payment info
    $orders_payments = $db->Execute("select payment_id, payment_number, card_expiry, payment_name, payment_type_full, payment_amount, date_posted, last_modified, purchase_order_id, comments  
                                    from " . TABLE_YO_PAYMENTS . " left join " . TABLE_YO_PAYMENT_TYPES . " on payment_type = payment_type_code 
                                    where orders_id = '" . zen_db_input($oID) . "'
                                    order by date_posted");

      while (!$orders_payments->EOF) {
        echo '        <tr  style="color:'. (($orders_payments->fields['payment_amount']>=0) ? 'black' : 'red' ) . '">' . "\n" .
             '            <td class="smallText" align="center">' . zen_date_short($orders_payments->fields['date_posted']) . '</td>' . "\n" .
             '     <!--       <td class="smallText" align="center">' . zen_datetime_short($orders_payments->fields['last_modified']) . '</td> -->' . "\n" .
//             '            <td class="smallText" align="center">' . $orders_payments->fields['payment_name'] . '</td>' . "\n" .
             '            <td class="smallText" align="center">' . $orders_payments->fields['payment_type_full'] . '</td>' . "\n" .
//             '            <td class="smallText" align="center">' . $orders_payments->fields['payment_number'] . '</td>' . "\n" .
//             '            <td class="smallText" align="center">' . $orders_payments->fields['card_expiry'] . '</td>' . "\n" .
             '            <td class="smallText" align="right">' . $orders_payments->fields['payment_amount'] . '</td>' . "\n" .
             '            <td class="smallText" align="right">' . $orders_payments->fields['comments'] . '&nbsp;</td>' . "\n";
        echo '            <td class="smallText" align="right"><a href="javascript:popupWindow(\'' .
                   zen_href_link('order_payments_del', 'oID=' . $oID . '&action=delete&payment_mode=payment&index=' . $orders_payments->fields['payment_id'] , 'NONSSL') . '\', \'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=no,width=650,height=450,screenX=150,screenY=100,top=100,left=150\')">' . BUTTON_DEL_PAYMENT . '</a></td>' . "\n" .
             '        </tr>' . "\n";
		$orders_payments_total += $orders_payments->fields['payment_amount'];
        $orders_payments->MoveNext();
      }
/*      echo '          <form name="add_payment" action="orders.php" method="post" > <tr>' . "\n" .
           '            <td class="smallText"><input type="hidden" name="order_id" value="' . $oID . '"><input type="text" name="payment_name"> </td>' . "\n" .
           '            <td class="smallText"><select name="payment_type">'.
    $orders_payment_types = $db->Execute("select payment_type_id, payment_type_code, payment_type_full 
                                    from " . TABLE_YO_PAYMENT_TYPES . "
                                    order by payment_type_full");

      while (!$orders_payment_types->EOF) {
		   '<option value="' . $orders_payment_types->fields['payment_type_code'] . '">' . $orders_payment_types->fields['payment_type_full'] . '</option>'.
        $orders_payment_types->MoveNext();
      }
		   '</select> </td>' . "\n" .
           '            <td class="smallText"> <input type="button" value="' . BUTTON_ADD_PAYMENT . '"></td>' . "\n" .
           '          </tr>' . "\n";
*/
	$order_balance = $order_total_amount - $orders_payments_total;
	$bal_color = "";
	if ($order_balance == 0) {
		$bal_color = "color: white; background-color: green;";
	}
	else if ($order_balance > 0) {
	    $bal_color =  "color: white; background-color: red;";
	}
	else {
		$bal_color =  "color: white; background-color: blue;";
	}

?>
			<tr><td colspan="5" align="right"><b> Amount Paid: <?php echo $currencies->format($orders_payments_total, true, $order->info['currency'], $order->info['currency_value']); ?> </b></td></tr>
			<tr><td colspan="5" align="right"  style=" <?php echo $bal_color; ?> font-size:+1.5em;"><b>  Order Balance: 
			   <?php 
			     echo $currencies->format($order_total_amount - $orders_payments_total, true, $order->info['currency'], $order->info['currency_value']); 
			   ?> </b></td></tr>
         </table>
         </td>
      </tr>
...
<?php
    ...
    if ($orders_history->RecordCount() > 0) {
      while (!$orders_history->EOF) {
        ...
        echo '            <td class="smallText"><table cellpadding=0 cellspacing=0 class="inv_comments"><tr><td>' . nl2br(zen_db_output($orders_history->fields['comments'])) . '&nbsp;</td></tr></table></td>' . "\n" .
             '          </tr>' . "\n";
...
?>
                ...
                <td class="dataTableHeadingContent" align="right"><?php echo "Due"; ?></td>
...
<?php
    if (isset($_GET['cID'])) {
      ...
      $orders_query_raw =   "select o.orders_id, o.customers_id, o.customers_name, o.payment_method, o.shipping_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, o.amount_paid, o.balance_due, o.orders_status, s.orders_status_name, ot.text as order_total, ot.value as total_value " .
                            $new_fields . "
                            from (" . TABLE_ORDERS . " o, " .
...
//echo '<BR><BR>I SEE B: ' . $orders_query_raw . '<BR><BR>';
      ...
      $orders_query_raw = "select " . $search_distinct . " o.orders_id, o.customers_id, o.customers_name, o.payment_method, o.shipping_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, o.amount_paid, o.balance_due, o.orders_status, s.orders_status_name, ot.text as order_total, ot.value as total_value " .
                          $new_fields . "
                          from (" . TABLE_ORDERS . " o, " .
                          TABLE_ORDERS_STATUS . " s " .
                          $new_table . ")
...
<?php 
				$o_total = $orders->fields['total_value'];
				$bal_due = $o_total - $orders->fields['amount_paid'];
//				$bal_due = $orders->fields['balance_due'];
				$bal_color = "";
				if ( $orders->fields['orders_status'] >= STATUS_ORDER_DELIVERED ) {
					if ($bal_due == 0) {
						$bal_color = " style='color: white; background-color: green;'";
					}
					else if ($bal_due > 0) {
					    $bal_color =  " style='color: white; background-color: red;'";
					}
					else {
						$bal_color =  " style='color: white; background-color: blue;'";
					}
				}
?>                
                <td class="dataTableContent" <?php echo $bal_color; ?> align="right"><?php echo $currencies->format($bal_due); ?></td>
                <td class="dataTableContent" align="center"><?php echo zen_datetime_short($orders->fields['date_purchased']); ?></td>
<?php 

      $products_query_raw = "select products_model " .
                          " from " . TABLE_ORDERS_PRODUCTS .
                          " where orders_id = " . $orders->fields['orders_id'] . 
						  " order by products_model ";

      $orders_products = $db->Execute($products_query_raw);
	  $product_list = "";
	  while (!$orders_products->EOF) {
		  $product_list = $product_list . $orders_products->fields['products_model'] . "&nbsp; ";  
          $orders_products->MoveNext();
	  }
?>

                <td class="dataTableContent" align="center"><?php echo $product_list; ?></td>
...
<?php echo '<a href="javascript:popupWindow(\'' .
                   zen_href_link('order_payments', 'oID=' . $orders->fields['orders_id'] . '&action=add&payment_mode=payment', 'NONSSL') . '\', \'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=no,width=650,height=450,screenX=150,screenY=100,top=100,left=150\')"> $ </a> &nbsp; <a href="' . zen_href_link(FILENAME_ORDER_EDIT, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $orders->fields['orders_id'] . '&action=edit', 'NONSSL') . '">' . zen_image(DIR_WS_IMAGES . 'icon_edit.gif', ICON_EDIT) . '</a>'; 
?>
<?php if (isset($oInfo) && is_object($oInfo) && ($orders->fields['orders_id'] == $oInfo->orders_id)) { echo zen_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . zen_href_link(FILENAME_ORDERS, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $orders->fields['orders_id'] . '&action=edit', 'NONSSL') . '">' . zen_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
      $orders->MoveNext();
    }
...
<?php
  ...
  switch ($action) {
    case 'delete':
      ...
      $contents = array('form' => zen_draw_form('orders_del', FILENAME_ORDERS_DEL, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=deleteconfirm', 'post', '', true));
...
//end of modifications of edit orders.
      }
      break;
  }
